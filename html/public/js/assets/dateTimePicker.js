/**
 * @@@
 * DateTimePicker / unAvailable Calendar (version)
 */
'use strict';
  ...
  if (typeof define === 'function' && define.amd) {
	...
	var plugin_name = 'calendar',
			...
			defaults = {
				...
				available: [], // static data - to merge with dynamic data
				adapter: null, // host to get json data of available date
	...
	Plugin.prototype = {
		...
		update: function(){
			...
			if (typeof this.options.adapter === 'string'){
				$.getJSON(this.options.adapter, {
					...
					_this.available = _this.options.available.concat(data);
					_this.draw(months);
				});
			}else{
							...
							for (var i = 0; i < total_week; i++){
								ap.push($('<tr>').append(function(){
									...
									for (var j = 0; j < 7; j++){
										...
										//var unavailable = me.options.filter.call(me, d);
										...
										.append(d.getDate()).wrapInner("<div class='dai'></div>").click(function(){
		 ...
		 * Base on this.available data / input MySql date format (support glob)
